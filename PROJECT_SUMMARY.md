...
- ✅ **成員組別轉換測試**（test_case_member_group_conversion）:
  - 測項1：建立一般組成員、新增路線、調整成員組別、驗證分數

- ✅ **成員和路線操作測試**（test_case_member_route_operations）:
  - 測項2：成員和路線的增刪操作對分數的影響

- ✅ **安全性測試**（test_case_security）:
  - 用戶認證測試（註冊、登錄、登出）
  - API 權限控制測試
  - XSS 攻擊防護測試
  - SQL 注入防護測試
...
- ✅ **用戶登錄**: `/api/auth/login/` - Session 認證和 Basic 認證
